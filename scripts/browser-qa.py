"""Production-build browser regression checks; no Browser plugin was available during creation.
...
with tempfile.TemporaryDirectory(prefix='auradio-qa-') as directory:
    ...
    try:
        with sync_playwright() as p:
            ...
            # All same-site links resolve to generated files and same-page fragment targets.
            ...
            page.goto(base+'experience/',wait_until='networkidle')
            ...
            assert page.evaluate('Boolean(document.fullscreenElement)') or page.locator('.audio-feedback').inner_text()
            ...
            rp.goto(base+'experience/',wait_until='networkidle')
            ...
            reduced.close()
            # Test the manual preference independently from the system override.
            ...
            fp=fallback.new_page();fp.goto(base,wait_until='networkidle')
            ...
            summary={'checks':checks,'count':len(checks),'browser':browser.version,'viewports':['1440x1000','390x844','360x844'],'errors':errors}
