...
打开终端打印的本机地址，默认 `http://127.0.0.1:4173/`。开发模式使用 `npm run dev`。
...
14 个领域/边界/服务器及部署配置测试。独立 CI 对生产构建完成 46 项真实浏览器检查，涵盖 1440、390、360 像素宽度和实际媒体交互。成功记录：https://github.com/KangQiovo/auradio_web/actions/runs/35584913697

Pages 工作流只部署 dist，不能把整个仓库作为网站根目录。正式官网：https://auradio.kangqiovo.com/ 。Astro、CNAME、robots 和 sitemap 使用已绑定域名的根路径 `/`；本地启动器同时兼容旧 `/auradio_web/` 入口。部署后检查真实 HTTPS 构建标识、页面、样式、脚本和音频，报告保存在 Actions 产物中。
