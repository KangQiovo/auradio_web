...
import {attachSignal,detachSignal,refreshSignal,useSignalKind} from '../lib/audio-signal';
...
export default function AudioStudio() {
  ...
  const signalOwner=useRef({});
  const signalKind=useSignalKind();
  ...
  useEffect(()=>{
    const leave=()=>{
      ...
      detachSignal(signalOwner.current);
  ...
  async function startPlayback() {
    ...
    try {
      if(!context.current && typeof AudioContext!=='undefined') {
        ...
        const analysis=c.createAnalyser();analysis.fftSize=2048;analysis.smoothingTimeConstant=0.35;
    ...
    if(analyser.current)attachSignal(signalOwner.current,analyser.current,()=>!a.paused&&!a.ended&&a.readyState>=3,'local');
  ...
  function selectTrack(next:Track,autoplay=false) {
    ...
    setTrack(next);
    if(!next.url)detachSignal(signalOwner.current);
  ...
  return <div className="audio-studio" ref={root}>
    <div className="studio">
      <div className="studio-view">
        <div className="studio-view-top"><span><i className={`status-dot ${playing?'on':''}`}/>{signalKind==='tab'?'标签页声音同步中':playing?'正在播放':'等待播放'}</span><button className="icon-button" onClick={()=>void fullscreen()} aria-label="切换体验全屏"><Icon name="expand"/></button></div>
        <SoundField mode={mode} intensity={intensity} animated={false}/>
        <SpectrumStrip/>
        <div className="studio-track"><div><span className="overline">LOCAL AUDIO / YOUR SOUND</span><h2>{signalKind==='tab'?'与当前试听同步':track.title}</h2><p>{signalKind==='tab'?'播放与暂停请使用上方官方播放器。':track.subtitle}</p></div><span className="studio-track-number" aria-hidden="true">L</span></div>
        <div className="transport" hidden={signalKind==='tab'}>
      ...
      <aside className="studio-controls" aria-label="声场设置">
        <span className="overline">按你的方式来</span><h3>调一调，<br/>看看声音。</h3><p>将真实低、中、高频分开呈现。上方开启声音同步或选择本地音频后，图案随当前声音变化；未播放时也可轻触拨散。形态与强度只改变画面，不改变声音。</p>
