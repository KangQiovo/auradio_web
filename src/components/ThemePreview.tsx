...
export default function ThemePreview() {
  ...
  const items=tab==='歌曲'?['慢慢靠近','日光之间','一段留白']:tab==='专辑'?['房间里的声音','白昼练习']:['Auradio Studio'];
  ...
  return <div className="theme-explorer">
    ...
    <div className={`phone-preview theme-${selected.id}`}>
      ...
      <div className="phone-heading"><span>你的曲库</span><span>在这台设备上</span></div>
      ...
      <AnimatePresence mode="wait" initial={false}><motion.div key={`${tab}-${selected.id}`} initial={{opacity:0,y:reduced?0:7}} animate={{opacity:1,y:0}} exit={{opacity:0}} transition={{duration:reduced?0:0.18}} className="library-items">
        {filtered.map((name,index)=><div key={name} className="library-item"><div className={`mini-cover cover-${index}`} aria-hidden="true"><span>{String(index+1).padStart(2,'0')}</span></div><div><b>{name}</b><span>{tab==='艺术家'?'网页示例艺术家':'Auradio Studio · 网页示例'}</span></div><span className="track-order">0{index+1}</span></div>)}
