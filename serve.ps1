    ...
    using(client) {
      try {
        ...
        using(NetworkStream stream=client.GetStream()) {
          ...
          if(path=="/"||path=="/auradio_web"){Header(stream,302,"text/plain",0,"Location: /auradio_web/\r\n");return;}
          if(!path.StartsWith("/auradio_web/",StringComparison.Ordinal)||path.IndexOf('\\')>=0||path.IndexOf('\0')>=0){Header(stream,404,"text/plain",0,"");return;}
          string relative=path.Substring(13);
...
$Url = "http://127.0.0.1:$ActualPort/auradio_web/"
